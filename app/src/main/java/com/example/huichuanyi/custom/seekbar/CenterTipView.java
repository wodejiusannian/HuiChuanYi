...
public class CenterTipView extends View {
    ...
    public void init(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            switch (attr) {
                case R.styleable.CenterTipView_seekbarbgColor:
                    //背景颜色
                    bgColor = typedArray.getColor(attr, Color.BLACK);
                    break;
                case R.styleable.CenterTipView_seekbartextColor:
                    //文本颜色
                    textColor = typedArray.getColor(attr, Color.WHITE);
                    break;
                case R.styleable.CenterTipView_seekbartext:
                    //文本内容
                    text = typedArray.getString(attr);
                    break;
                case R.styleable.CenterTipView_seekbartype:
                    //图形类型
                    type = typedArray.getInt(R.styleable.CenterTipView_seekbartype, 0);
                    break;
                case R.styleable.CenterTipView_seekbartextSize:
                    //字体大小
